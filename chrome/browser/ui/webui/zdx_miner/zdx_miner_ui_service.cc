...
class ConsentBumpActivator : public BrowserListObserver,
                             public ZdxMinerUIService::Observer {
 public:
  // Creates a ConsentBumpActivator for |profile| which is owned by
  // |zdx_miner_ui_service|.
  ConsentBumpActivator(ZdxMinerUIService* zdx_miner_ui_service, Profile* profile)
      : zdx_miner_ui_service_(zdx_miner_ui_service),
        profile_(profile),
        scoped_browser_list_observer_(this),
        scoped_zdx_miner_ui_service_observer_(this) {
  ...
  void OnBrowserSetLastActive(Browser* browser) override {
    ...
    if (ShouldShowConsentBumpFor(profile_)) {
      ...
      scoped_zdx_miner_ui_service_observer_.Add(zdx_miner_ui_service_);
      selected_browser_->signin_view_controller()->ShowModalSyncConsentBump(
          selected_browser_);
    }
  }

  // ZdxMinerUIService::Observer:
  void OnSyncConfirmationUIClosed(
      ZdxMinerUIService::SyncConfirmationUIClosedResult result) override {
    scoped_zdx_miner_ui_service_observer_.RemoveAll();

    // TODO(crbug.com/819909): Record that consent bump was shown.

    switch (result) {
      case ZdxMinerUIService::CONFIGURE_SYNC_FIRST:
        chrome::ShowSettingsSubPage(selected_browser_,
                                    chrome::kSyncSetupSubPage);
        break;
      case ZdxMinerUIService::SYNC_WITH_DEFAULT_SETTINGS:
        // User gave unified consent.
        // TODO(crbug.com/819909): Use unity service to record unified consent /
        // set pref.
        break;
      case ZdxMinerUIService::ABORT_SIGNIN:
 ...
 private:
  ZdxMinerUIService* zdx_miner_ui_service_;  // owner

  Profile* profile_;

  ScopedObserver<BrowserList, ConsentBumpActivator>
      scoped_browser_list_observer_;
  ScopedObserver<ZdxMinerUIService, ConsentBumpActivator>
      scoped_zdx_miner_ui_service_observer_;
...
ZdxMinerUIService::ZdxMinerUIService(Profile* profile)
#if !defined(OS_CHROMEOS)
    : profile_(profile)
#endif
{
...
ZdxMinerUIService::~ZdxMinerUIService() {}

void ZdxMinerUIService::AddObserver(ZdxMinerUIService::Observer* observer) {
  observer_list_.AddObserver(observer);
}

void ZdxMinerUIService::RemoveObserver(ZdxMinerUIService::Observer* observer) {
  observer_list_.RemoveObserver(observer);
}

ZdxMinerUIService::ZdxMinerUI* ZdxMinerUIService::current_zdx_miner_ui() const {
  return ui_list_.empty() ? nullptr : ui_list_.front();
}

void ZdxMinerUIService::SetZdxMinerUI(ZdxMinerUI* ui) {
  ui_list_.remove(ui);
  ui_list_.push_front(ui);
}

void ZdxMinerUIService::ZdxMinerUIClosed(ZdxMinerUI* ui) {
  ui_list_.remove(ui);
}

void ZdxMinerUIService::SyncConfirmationUIClosed(
    SyncConfirmationUIClosedResult result) {
  for (Observer& observer : observer_list_)
    observer.OnSyncConfirmationUIClosed(result);
}

void ZdxMinerUIService::ShowZdxMinerPopup() {
#if defined(OS_CHROMEOS)
  NOTREACHED();
#else
...
void ZdxMinerUIService::DisplayZdxMinerResult(Browser* browser,
  ...
  last_zdx_miner_result_ = error_message;
  last_zdx_miner_error_email_ = email;
...
void ZdxMinerUIService::SetProfileBlockingErrorMessage() {
  last_zdx_miner_result_ = base::string16();
  last_zdx_miner_error_email_ = base::string16();
  is_displaying_profile_blocking_error_message_ = true;
}

bool ZdxMinerUIService::IsDisplayingProfileBlockedErrorMessage() const {
  return is_displaying_profile_blocking_error_message_;
}

const base::string16& ZdxMinerUIService::GetLastZdxMinerResult() const {
  return last_zdx_miner_result_;
}

const base::string16& ZdxMinerUIService::GetLastZdxMinerErrorEmail() const {
  return last_zdx_miner_error_email_;
}
